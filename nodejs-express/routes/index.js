...
router.post('/salesDate', function(req, res) {
    fs.readFile(StatisticsSales, function(err, data) {
        ...
        var salesDateData = salesDateData = JSON.parse(data);
...
//select video
router.get('/getdata_video', function(req, res, next) {

    conn.query('SELECT * FROM videos', (error, response) => {
            if (error) {
                console.log(error);
            } else {
                res.send(response)
...
router.post('/add_product', function(req, res, next) {
    ...
    conn.query("INSERT INTO all_product (id,productName,productAmount,productPrice,productComplatePrice,productDescribe,productImage,productImageList," +
            ...
            "','" + productImageList + "','" + productCategory + "','" + productReducedPrice + "','" + productContent +
...
router.post('/edit_product', function(req, res, next) {
        ...
        if (productImageList == undefined) {
            productImageList = imageListTemp;
        }


        var sql = "UPDATE all_product set productName =? ,productAmount =? , productPrice =? , productComplatePrice=? , productDescribe =? ," +
            "productImage =? ,productImageList =? , productCategory =? , productReducedPrice =? , productContent =? , productGuarantee =? ," +
            "productCategoryCode =? , productTitle =? , productKeyword =? , dateTime =?, colors=?, productSizes=?, productStorageCapacity=?  WHERE id = ?";

        conn.query(sql, [productName, productAmount, productPrice, productComplatePrice, productDescribe, productImage, productImageList, productCategory,
            productReducedPrice, productContent, productGuarantee, productCategoryCode, productTitle,
            productKeyword, dateTime, pushColors, productSizes, productStorageCapacity, productId
        ], function(err, result) {
