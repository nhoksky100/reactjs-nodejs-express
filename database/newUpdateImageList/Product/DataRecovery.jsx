...
class DataRecovery extends Component {
                ...
                return (
                    <tr className="row_9" key={key}>
                        ...
                        <td className="option textC">
                            ...
                            <a href='#recover' title="phục hồi dữ liệu" name='id'
                                style={{ marginRight: '30px' }}
                                className="tipS verify_action" onClick={() => this.recovery(value, false)}>
                                <img src="images/icons/color/plus.png" alt='' /></a>
                            < a href='#delete' title="Xóa" name='id' className="tipS verify_action ml5" onClick={() => this.delete(value.id, true)}>
    ...
    recovery = (data, isRecovery) => {
        ...
        let { id, productName, productAmount, productPrice, productComplatePrice, productDescribe, productImage, productImageList, productCategory, productSizes,
    ...
    //xóa
    delete = (id, isFlag, isRecovery) => {
        let dataRecovery = [];

        dataRecovery = this.state.dataTemp.filter((item) => item.id !== id);
        ...
        axios.post('/removeDataRecovery', { dataRecovery })
            .then((response) => {
                ...
                if (isFlag) {
                    let isDeleteImageList = true
                    let listFileImage = ''
                    axios.post('/updateListImage', { id, listFileImage, isDeleteImageList })
                    toast(<div className="advertise"><i className="fa fa-minus-circle" aria-hidden="true" />
    ...
    searchClick = () => {
        let { search_value, dateTimeStart, dateTimeEnd } = this.props;
        let dataProductSearch = [], dataProductDate = null;
